...
function(_ir_update_destination folder)
  if(NOT DEFINED ${folder})
    ...
    set(ir_REMOTE_DESTINATION OFF PARENT_SCOPE)
  else()
    set(ifolder "${${folder}}")
    if(IS_ABSOLUTE ifolder)
      cmake_path(NORMALIZE ifolder)
      set(ir_REMOTE_DESTINATION ON PARENT_SCOPE)
    else()
...
function(_ir_handle_roots ir_target root)
  ...
  set(cpp_global_flags "-feliminate-unused-debug-symbols -S")
  if(NOT ir_ASM)
    set(cpp_global_flags "${cpp_global_flags} -emit-llvm")
  endif()
  set(cpp_global_flags "${cpp_global_flags} ${CMAKE_CXX_FLAGS} ${${CMAKE_CXX_FLAGS_mode}}")
  if(ir_OPTLEVEL)
  ...
  set(ir_compile_commands "${ir_DESTINATION}/CompileCommands.${_ir_script_ext}")
  _ir_script_init("${ir_compile_commands}")
  if(NOT ir_REMOTE_DESTINATION)
    file(APPEND "${ir_compile_commands}"
     "rm ${ir_DESTINATION}/*.ll\nrm ${ir_DESTINATION}/*.bc\nrm ${ir_DESTINATION}/*.asm\n\n")
  endif()
  ...
  foreach(cpp_target ${ir_cpp_targets})
    ...
    list(FILTER cpp_options EXCLUDE REGEX "^(-g)|(-W[a-zA-Z0-9_-]+)")
...
function(compile_ir target_name)
  ...
  set(ir_opts   BITCODE ASM NODEBUG)
  ...
  if(DEFINED ir_BITCODE AND ir_BITCODE)
    if(DEFINED ir_ASM AND ir_ASM)
      _ir_throw("Cannot provide BITCODE and ASM together")
    else()
      set(ir_extension "bc")
    endif()
  elseif(DEFINED ir_ASM AND ir_ASM)
    set(ir_extension "asm")
  else()
    set(ir_extension "ll")
  endif()
