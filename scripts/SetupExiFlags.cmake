...
if(WIN32 AND EXI_ON_CLANG)
  set(EXI_CLANG_PDB_FLAGS "-g;-Wl,--pdb=")
  if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.18.0")
    include(CheckLinkerFlag)
    # CMAKE_REQUIRED_QUIET?
    check_linker_flag(CXX "${EXI_CLANG_PDB_FLAGS}" HAVE_PDB_FLAGS)
  ...
  if(HAVE_PDB_FLAGS EQUAL 1)
    set(EXI_PDB_FLAGS "${EXI_CLANG_PDB_FLAGS}")
  endif()
else()
  set(HAVE_PDB_FLAGS 0)
...
if(HAVE_PDB_FLAGS)
  ...
  macro(_exi_add_pdb target target_type)
    get_target_property(target_has_pdb ${target} EXI_PDB_ADDED)
    if(NOT target_has_pdb)
      message(VERBOSE "Adding PDB to ${target}")
      if(${target_type} MATCHES "EXECUTABLE|MODULE_LIBRARY")
        target_link_options(${target} PUBLIC ${EXI_PDB_FLAGS})
        set_target_properties(${target} PROPERTIES EXI_PDB_ADDED 1)
      ...
      elseif(${target_type} STREQUAL "INTERFACE_LIBRARY")
        ...
        set_target_properties(${target} PROPERTIES EXI_PDB_ADDED 0)
      endif()
    endif()
  endmacro(_exi_add_pdb)
else()
  macro(_exi_add_pdb target target_type)
    message(VERBOSE "Not adding PDB to ${target} (not defined)")
    set_target_properties(${target} PROPERTIES EXI_PDB_ADDED 0)
  endmacro(_exi_add_pdb)
...
#  exi_add_target_options(<target> [PDB|ADD_PDB])
function(exi_add_target_options target)
  cmake_parse_arguments(arg "PDB;ADD_PDB" "" "" ${ARGN})
  ...
  # Add PDB
  if(arg_PDB OR arg_ADD_PDB)
    _exi_add_pdb(${target} ${target_type})
  endif(arg_PDB OR arg_ADD_PDB)
...
macro(exi_add_executable target)
  ...
  target_compile_options(${target} PUBLIC ${EXI_FLAGS})
  if(arg_PDB OR arg_ADD_PDB)
    _exi_add_pdb(${target} "EXECUTABLE")
  endif(arg_PDB OR arg_ADD_PDB)
