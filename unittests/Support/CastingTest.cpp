//===- unittests/Support/CastingTest.hpp ----------------------------===//
//
...
// TODO: Remove -Wdeprecated
CLANG_IGNORED("-Wdeprecated")
...
TEST(CastingTest, dyn_cast_if_present) {
  Option<T1> empty{};
  Option<T2> F1 = dyn_cast_if_present<T2>(empty);
  EXPECT_FALSE(F1.has_value());

  T1 t1;
  Option<T2> F2 = dyn_cast_if_present<T2>(t1);
