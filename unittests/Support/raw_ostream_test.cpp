//===- unittests/Support/raw_ostream_test.hpp -----------------------===//
//
...
#include <core/Config/Config.inc> // for EXI_ON_UNIX
#include <core/Support/Errc.hpp>
#include <core/Support/Filesystem.hpp>
//#include <core/Support/FileUtilities.hpp>
#include <core/Support/Format.hpp>
#include <core/Support/MemoryBuffer.hpp>
#include <core/Support/raw_ostream.hpp>
//#include "core/Testing/Support/Error.hpp"
#include "gtest/gtest.h"
#include <optional>

using namespace exi;

// TODO: Add FileRemover
#define HAS_FILEREMOVER 0

namespace {

template <typename T> std::string printToString(const T &Value) {
  std::string res;
  exi::raw_string_ostream OS(res);
  OS.SetBuffered();
  OS << Value;
  OS.flush();
  return res;
...
template<typename T> std::string printToString(const T &Value,
  ...
  exi::raw_svector_ostream OS(SVec);
...
#endif // HAS_FILEREMOVER

static void checkFileData(StrRef FileName, StrRef GoldenData) {
...
TEST(raw_ostreamTest, filePermissions) {
  ...
  exi::unittest::TempDir RootTestDirectory("writToOutput", /*Unique*/ true);
  SmallStr<128> Path(RootTestDirectory.path());
  ...
  ErrorOr<exi::sys::fs::perms> Perms = exi::sys::fs::getPermissions(Path);
  ...
  EXPECT_EQ(Perms.get(), exi::sys::fs::all_read | exi::sys::fs::all_write);

  ::umask(OldMask);
}
#endif

// TODO: Add Testing/Support/Error.hpp

#if 0
TEST(raw_ostreamTest, writeToNonexistingPath) {
  StrRef FileName = "/_bad/_path";
