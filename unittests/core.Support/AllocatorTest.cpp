//===- unittests/Support/AllocatorTest.hpp --------------------------===//
//
...
TEST(AllocatorTest, TestIdentifyObject) {
  ...
  u64 *a = (u64 *)Alloc.Allocate(sizeof(u64), alignof(u64));
  Option<i64> maybe_a_belongs = Alloc.identifyObject(a);
  EXPECT_TRUE(maybe_a_belongs.has_value());
  EXPECT_TRUE(*maybe_a_belongs >= 0);

  u64 *b = nullptr;
  Option<i64> maybe_b_belongs = Alloc.identifyObject(b);
  EXPECT_FALSE(maybe_b_belongs);

  // The default slab size is 4096 (or 512 u64 values). Custom slabs are
  // allocated when the requested size is larger than the slab size.
  u64 *c =
      (u64 *)Alloc.Allocate(sizeof(u64) * 1024, alignof(u64));
  Option<i64> maybe_c_belongs = Alloc.identifyObject(c);
...
public:
  ...
  void Deallocate(void *Slab, usize /*Size*/, usize /*Alignment*/) {
    exi_free(((void**)Slab)[-1]);
  }

  static usize GetLastSlabSize() { return LastSlabSize; }
};

usize MockSlabAllocator::LastSlabSize = 0;
