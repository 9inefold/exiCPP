...
using exi::itanium_demangle::OutputBuffer;
using exi::itanium_demangle::ScopedOverride;
using exi::itanium_demangle::starts_with;
...
char *exi::rustDemangle(std::string_view MangledName) {
  ...
  if (!D.demangle(MangledName)) {
    exi::exi_free(D.Output.getBuffer());
    return nullptr;
  }
