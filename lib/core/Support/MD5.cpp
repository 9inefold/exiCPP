...
const u8 *MD5::body(ArrayRef<u8> Data) {
  const u8 *ptr;
  MD5_u32plus a, b, c, d;
...
void MD5::update(ArrayRef<u8> Data) {
  ...
  if (used) {
    ...
    if (Size < free) {
      std::memcpy(&InternalState.buffer[used], Ptr, Size);
      return;
    }

    std::memcpy(&InternalState.buffer[used], Ptr, free);
    Ptr = Ptr + free;
    Size -= free;
  ...
  std::memcpy(InternalState.buffer, Ptr, Size);
}

/// Add the bytes in the StrRef \p Str to the hash.
// Note that this isn't a string and so this won't include any trailing NULL
// bytes.
void MD5::update(StrRef Str) {
  ArrayRef<u8> SVal((const u8 *)Str.data(), Str.size());
  update(SVal);
}
...
void MD5::final(MD5Result &Result) {
  ...
  if (free < 8) {
    std::memset(&InternalState.buffer[used], 0, free);
    body(ArrayRef(InternalState.buffer, 64));
    used = 0;
    free = 64;
  }

  std::memset(&InternalState.buffer[used], 0, free - 8);
...
SmallStr<32> MD5::MD5Result::digest() const {
  SmallStr<32> Str;
  toHex(*this, /*LowerCase*/ true, Str);
  return Str;
}

void MD5::stringifyResult(MD5Result &Result, SmallVecImpl<char> &Str) {
  toHex(Result, /*LowerCase*/ true, Str);
}

MD5::MD5Result MD5::hash(ArrayRef<u8> Data) {
  MD5 Hash;
  Hash.update(Data);
  MD5::MD5Result Res;
  Hash.final(Res);
