...
bool ConvertUTF8toWide(unsigned WideCharWidth, exi::StrRef Source,
                       char *&ResultPtr, const UTF8 *&ErrorPtr) {
  exi_assert(WideCharWidth == 1 || WideCharWidth == 2 || WideCharWidth == 4);
  ...
  exi_assert((result != targetExhausted),
...
bool convertUTF16ToUTF8String(ArrayRef<char> SrcBytes, std::string &Out) {
  ...
  exi_assert(isAddrAligned(Align::Of<UTF16>(), Src));
...
bool convertUTF16ToUTF8String(ArrayRef<UTF16> Src, std::string &Out) {
  return convertUTF16ToUTF8String(
      exi::ArrayRef<char>(reinterpret_cast<const char *>(Src.data()),
...
bool convertUTF32ToUTF8String(ArrayRef<char> SrcBytes, std::string &Out) {
  ...
  exi_assert((uintptr_t)Src % sizeof(UTF32) == 0);
...
bool convertUTF8ToUTF16String(StrRef SrcUTF8,
                              SmallVecImpl<UTF16> &DstUTF16) {
  exi_assert(DstUTF16.empty());
...
static inline bool ConvertUTF8toWideInternal(exi::StrRef Source,
                                             TResult &Result) {
...
bool ConvertUTF8toWide(exi::StrRef Source, std::wstring &Result) {
...
bool ConvertUTF8toWide(const char *Source, std::wstring &Result) {
  ...
  return ConvertUTF8toWide(exi::StrRef(Source), Result);
}
...
bool convertWideToUTF8(const std::wstring &Source, std::string &Result) {
  if (sizeof(wchar_t) == 1) {
    ...
    return convertUTF16ToUTF8String(
        exi::ArrayRef<UTF16>(reinterpret_cast<const UTF16 *>(Source.data()),
                              Source.size()),
        Result);
