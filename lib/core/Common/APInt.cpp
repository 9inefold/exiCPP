...
inline static unsigned getDigit(char cdigit, u8 radix) {
  unsigned r;
...
void APInt::initSlowCase(u64 val, bool isSigned) {
  if (isSigned && i64(val) < 0) {
    U.pVal = getMemory(getNumWords());
    U.pVal[0] = val;
    memset(&U.pVal[1], 0xFF, kAPIntWordSize * (getNumWords() - 1));
    clearUnusedBits();
  } else {
...
void APInt::initSlowCase(const APInt& that) {
  U.pVal = getMemory(getNumWords());
  memcpy(U.pVal, that.U.pVal, getNumWords() * kAPIntWordSize);
}

void APInt::initFromArray(ArrayRef<u64> bigVal) {
  exi_assert(bigVal.data(), "Null pointer detected!");
  if (isSingleWord())
    U.VAL = bigVal[0];
  else {
    ...
    memcpy(U.pVal, bigVal.data(), words * kAPIntWordSize);
  }
  // Make sure unused high bits are cleared
  clearUnusedBits();
}

APInt::APInt(unsigned numBits, ArrayRef<u64> bigVal) : BitWidth(numBits) {
  initFromArray(bigVal);
}

APInt::APInt(unsigned numBits, unsigned numWords, const u64 bigVal[])
    : BitWidth(numBits) {
  initFromArray(ArrayRef(bigVal, numWords));
}

APInt::APInt(unsigned numbits, StrRef Str, u8 radix)
    : BitWidth(numbits) {
...
void APInt::assignSlowCase(const APInt &RHS) {
  ...
  if (isSingleWord())
    ...
    memcpy(U.pVal, RHS.U.pVal, getNumWords() * kAPIntWordSize);
}
...
void APInt::andAssignSlowCase(const APInt &RHS) {
  ...
  for (usize i = 0, e = getNumWords(); i != e; ++i)
    dst[i] &= rhs[i];
}
...
int APInt::compare(const APInt& RHS) const {
  exi_assert(BitWidth == RHS.BitWidth, "Bit widths must be same for comparison");
  if (isSingleWord())
...
int APInt::compareSigned(const APInt& RHS) const {
  exi_assert(BitWidth == RHS.BitWidth, "Bit widths must be same for comparison");
  if (isSingleWord()) {
    i64 lhsSext = SignExtend64(U.VAL, BitWidth);
    i64 rhsSext = SignExtend64(RHS.U.VAL, BitWidth);
...
void APInt::setBitsSlowCase(unsigned loBit, unsigned hiBit) {
  ...
  u64 loMask = kWordTypeMax << whichBit(loBit);

  // If hiBit is not aligned, we need a high mask.
  unsigned hiShiftAmt = whichBit(hiBit);
  if (hiShiftAmt != 0) {
    // Create a high mask with zeros above hiBit.
    u64 hiMask = kWordTypeMax >> (kAPIntBitsPerWord - hiShiftAmt);
...
void APInt::flipBit(unsigned bitPosition) {
  exi_assert(bitPosition < BitWidth, "Out of the bit-width range!");
...
void APInt::insertBits(const APInt &subBits, unsigned bitPosition) {
  ...
  exi_assert((subBitWidth + bitPosition) <= BitWidth, "Illegal bit insertion");
  ...
  if (isSingleWord()) {
    u64 mask = kWordTypeMax >> (kAPIntBitsPerWord - subBitWidth);
  ...
  if (loBit == 0) {
    ...
    unsigned numWholeSubWords = subBitWidth / kAPIntBitsPerWord;
    memcpy(U.pVal + loWord, subBits.getRawData(),
           numWholeSubWords * kAPIntWordSize);

    // Mask+insert remaining bits.
    unsigned remainingBits = subBitWidth % kAPIntBitsPerWord;
    if (remainingBits != 0) {
      u64 mask = kWordTypeMax >> (kAPIntBitsPerWord - remainingBits);
      U.pVal[hi1Word] &= ~mask;
...
void APInt::insertBits(u64 subBits, unsigned bitPosition, unsigned numBits) {
  u64 maskBits = maskTrailingOnes<u64>(numBits);
  subBits &= maskBits;
  if (isSingleWord()) {
...
APInt APInt::extractBits(unsigned numBits, unsigned bitPosition) const {
  exi_assert(bitPosition < BitWidth && (numBits + bitPosition) <= BitWidth,
  ...
  u64 *DestPtr = Result.isSingleWord() ? &Result.U.VAL : Result.U.pVal;
  for (unsigned word = 0; word < NumDstWords; ++word) {
    u64 w0 = U.pVal[loWord + word];
    u64 w1 =
        (loWord + word + 1) < NumSrcWords ? U.pVal[loWord + word + 1] : 0;
    DestPtr[word] = (w0 >> loBit) | (w1 << (kAPIntBitsPerWord - loBit));
  }

  return Result.clearUnusedBits();
}

u64 APInt::extractBitsAsZExtValue(unsigned numBits,
                                       unsigned bitPosition) const {
  exi_assert(bitPosition < BitWidth && (numBits + bitPosition) <= BitWidth,
         "Illegal bit extraction");
  exi_assert(numBits <= 64, "Illegal bit extraction");

  u64 maskBits = maskTrailingOnes<u64>(numBits);
  if (isSingleWord())
    return (U.VAL >> bitPosition) & maskBits;

  static_assert(kAPIntBitsPerWord >= 64,
  ...
  u64 retBits = U.pVal[loWord] >> loBit;
  retBits |= U.pVal[hiWord] << (kAPIntBitsPerWord - loBit);
  retBits &= maskBits;
  return retBits;
}

unsigned APInt::getSufficientBitsNeeded(StrRef Str, u8 Radix) {
  exi_assert(!Str.empty(), "Invalid string length");
  usize StrLen = Str.size();
  ...
  if (Str[0] == '-' || Str[0] == '+') {
    IsNegative = Str[0] == '-';
    StrLen--;
    exi_assert(StrLen, "String is only a sign, needs a value.");
  }
...
unsigned APInt::getBitsNeeded(StrRef str, u8 radix) {
  // Compute a sufficient number of bits that is always large enough but might
  // be too large.
  ...
  // Each computation below needs to know if it's negative.
  StrRef::iterator p = str.begin();
  unsigned isNegative = *p == '-';
  if (*p == '-' || *p == '+') {
    p++;
    slen--;
    exi_assert(slen, "String is only a sign, needs a value.");
  }


  // Convert to the actual binary value.
  APInt tmp(sufficient, StrRef(p, slen), radix);
...
hash_code exi::hash_value(const APInt &Arg) {
  if (Arg.isSingleWord())
...
bool APInt::isSplat(unsigned SplatSizeInBits) const {
  exi_assert(getBitWidth() % SplatSizeInBits == 0,
...
APInt APInt::getSplat(unsigned NewLen, const APInt &V) {
  exi_assert(NewLen >= V.getBitWidth(), "Can't splat to smaller bit width!");
...
unsigned APInt::countLeadingZerosSlowCase() const {
  ...
  for (int i = getNumWords()-1; i >= 0; --i) {
    u64 V = U.pVal[i];
    if (V == 0)
      Count += kAPIntBitsPerWord;
    else {
      Count += exi::countl_zero(V);
      break;
    }
  }
  // Adjust for unused bits in the most significant word (they are zero).
  unsigned Mod = BitWidth % kAPIntBitsPerWord;
  Count -= Mod > 0 ? kAPIntBitsPerWord - Mod : 0;
  return Count;
}

unsigned APInt::countLeadingOnesSlowCase() const {
  unsigned highWordBits = BitWidth % kAPIntBitsPerWord;
  unsigned shift;
  if (!highWordBits) {
    highWordBits = kAPIntBitsPerWord;
    shift = 0;
  } else {
    shift = kAPIntBitsPerWord - highWordBits;
  }
  int i = getNumWords() - 1;
  unsigned Count = exi::countl_one(U.pVal[i] << shift);
  if (Count == highWordBits) {
    for (i--; i >= 0; --i) {
      if (U.pVal[i] == kWordTypeMax)
        Count += kAPIntBitsPerWord;
      else {
        Count += exi::countl_one(U.pVal[i]);
        break;
      }
    }
  }
...
unsigned APInt::countTrailingZerosSlowCase() const {
  ...
  for (; i < getNumWords() && U.pVal[i] == 0; ++i)
    Count += kAPIntBitsPerWord;
  if (i < getNumWords())
    Count += exi::countr_zero(U.pVal[i]);
...
unsigned APInt::countTrailingOnesSlowCase() const {
  unsigned Count = 0;
  unsigned i = 0;
  for (; i < getNumWords() && U.pVal[i] == kWordTypeMax; ++i)
    Count += kAPIntBitsPerWord;
  if (i < getNumWords())
    Count += exi::countr_one(U.pVal[i]);
...
APInt APInt::byteSwap() const {
  exi_assert(BitWidth >= 16 && BitWidth % 8 == 0, "Cannot byteswap!");
  if (BitWidth == 16)
    return APInt(BitWidth, exi::byteswap<u16>(U.VAL));
  if (BitWidth == 32)
    return APInt(BitWidth, exi::byteswap<u32>(U.VAL));
  if (BitWidth <= 64) {
    u64 Tmp1 = exi::byteswap<u64>(U.VAL);
    Tmp1 >>= (64 - BitWidth);
    return APInt(BitWidth, Tmp1);
  }

  APInt Result(getNumWords() * kAPIntBitsPerWord, 0);
  for (unsigned I = 0, N = getNumWords(); I != N; ++I)
    Result.U.pVal[I] = exi::byteswap<u64>(U.pVal[N - I - 1]);
...
APInt APInt::reverseBits() const {
  switch (BitWidth) {
  case 64:
    return APInt(BitWidth, exi::reverseBits<u64>(U.VAL));
  case 32:
    return APInt(BitWidth, exi::reverseBits<u32>(U.VAL));
  case 16:
    return APInt(BitWidth, exi::reverseBits<u16>(U.VAL));
  case 8:
    return APInt(BitWidth, exi::reverseBits<u8>(U.VAL));
  case 0:
    return *this;
  default:
    break;
...
APInt exi::APIntOps::GreatestCommonDivisor(APInt A, APInt B) {
...
APInt exi::APIntOps::RoundDoubleToAPInt(double Double, unsigned width) {
  u64 I = bit_cast<u64>(Double);

  // Get the sign bit from the highest order bit
  bool isNeg = I >> 63;

  // Get the 11-bit exponent and adjust for the 1023 bit bias
  i64 exp = ((I >> 52) & 0x7ff) - 1023;
  ...
  u64 mantissa = (I & (~0ULL >> 12)) | 1ULL << 52;
...
double APInt::roundToDouble(bool isSigned) const {
  ...
  // Handle the simple case where the value is contained in one u64.
  // It is wrong to optimize getWord(0) to VAL; there might be more than one word.
  if (isSingleWord() || getActiveBits() <= kAPIntBitsPerWord) {
    if (isSigned) {
      i64 sext = SignExtend64(getWord(0), BitWidth);
      return double(sext);
    } else
  ...
  if (hiWord == 0) {
    ...
    exi_assert(hiWord > 0, "huh?");
    u64 hibits = Tmp.U.pVal[hiWord] << (52 - n % kAPIntBitsPerWord);
    u64 lobits = Tmp.U.pVal[hiWord-1] >> (11 + n % kAPIntBitsPerWord);
    mantissa = hibits | lobits;
  }

  // The leading bit of mantissa is implicit, so get rid of it.
  u64 sign = isNeg ? (1ULL << (kAPIntBitsPerWord - 1)) : 0;
  u64 I = sign | (exp << 52) | mantissa;
  return bit_cast<double>(I);
}

// Truncate to new width.
APInt APInt::trunc(unsigned width) const {
  exi_assert(width <= BitWidth, "Invalid APInt Truncate request");

  if (width <= kAPIntBitsPerWord)
  ...
  // Copy full words.
  unsigned i;
  for (i = 0; i != width / kAPIntBitsPerWord; i++)
    Result.U.pVal[i] = U.pVal[i];

  // Truncate and copy any partial word.
  unsigned bits = (0 - width) % kAPIntBitsPerWord;
  if (bits != 0)
...
APInt APInt::sext(unsigned Width) const {
  exi_assert(Width >= BitWidth, "Invalid APInt SignExtend request");
  ...
  // Copy words.
  std::memcpy(Result.U.pVal, getRawData(), getNumWords() * kAPIntWordSize);
  ...
  Result.U.pVal[getNumWords() - 1] =
      SignExtend64(Result.U.pVal[getNumWords() - 1],
                   ((BitWidth - 1) % kAPIntBitsPerWord) + 1);

  // Fill with sign bits.
  std::memset(Result.U.pVal + getNumWords(), isNegative() ? -1 : 0,
              (Result.getNumWords() - getNumWords()) * kAPIntWordSize);
  Result.clearUnusedBits();
  return Result;
}
...
APInt APInt::zext(unsigned width) const {
  exi_assert(width >= BitWidth, "Invalid APInt ZeroExtend request");
...
void APInt::ashrSlowCase(unsigned ShiftAmt) {
  ...
  unsigned WordShift = ShiftAmt / kAPIntBitsPerWord;
  unsigned BitShift = ShiftAmt % kAPIntBitsPerWord;
...
unsigned APInt::nearestLogBase2() const {
  ...
  // get 0. If VAL is 0, we get kWordTypeMax which gets truncated to
  // UINT32_MAX.
  if (BitWidth == 1)
    return U.VAL - 1;
...
// back to a u64 which is then used to construct the result. Finally,
...
APInt APInt::sqrt() const {
  ...
  if (magnitude < 52) {
    return APInt(BitWidth,
                 u64(::round(::sqrt(double(isSingleWord() ? U.VAL
                                                               : U.pVal[0])))));
  }
  ...
  if (this->ult(square))
    return x_old;
  exi_assert(this->ule(nextSquare), "Error in APInt::sqrt computation");
...
static void KnuthDiv(u32 *u, u32 *v, u32 *q, u32* r,
                     unsigned m, unsigned n) {
  exi_assert(u, "Must provide dividend");
  exi_assert(v, "Must provide divisor");
  exi_assert(q, "Must provide quotient");
  exi_assert(u != v && u != q && v != q, "Must use different memory");
  exi_assert(n>1, "n must be > 1");
...
#ifdef KNUTH_DEBUG
#define DEBUG_KNUTH(X) DEBUG_ONLY(X)
#else
#define DEBUG_KNUTH(X) do {} while(false)
  ...
  // be of length m+n+1.
  unsigned shift = exi::countl_zero(v[n - 1]);
  u32 v_carry = 0;
  u32 u_carry = 0;
  if (shift) {
  ...
  do {
    ...
    // too large.
    u64 dividend = Make_64(u[j+n], u[j+n-1]);
    ...
    i64 borrow = 0;
    for (unsigned i = 0; i < n; ++i) {
      u64 p = u64(qp) * u64(v[i]);
      i64 subres = i64(u[j+i]) - borrow - Lo_32(p);
      u[j+i] = Lo_32(subres);
...
void APInt::divide(const WordType *LHS, unsigned lhsWords, const WordType *RHS,
  ...
  exi_assert(lhsWords >= rhsWords, "Fractional result");
  ...
  memset(Q, 0, (m+n) * sizeof(u32));
  if (Remainder)
    memset(R, 0, n * sizeof(u32));
  ...
  exi_assert(n != 0, "Divide by zero?");
  if (n == 1) {
    u32 divisor = V[0];
    u32 remainder = 0;
    for (int i = m; i >= 0; i--) {
      u64 partial_dividend = Make_64(remainder, U[i]);
...
APInt APInt::udiv(const APInt &RHS) const {
  ...
  if (isSingleWord()) {
    exi_assert(RHS.U.VAL != 0, "Divide by zero?");
  ...
  exi_assert(rhsWords, "Divided by zero???");
...
APInt APInt::udiv(u64 RHS) const {
  exi_assert(RHS != 0, "Divide by zero?");
...
APInt APInt::sdiv(i64 RHS) const {
  if (isNegative()) {
    if (RHS < 0)
      return (-(*this)).udiv(-RHS);
    return -((-(*this)).udiv(RHS));
...
APInt APInt::urem(const APInt &RHS) const {
  exi_assert(BitWidth == RHS.BitWidth, "Bit widths must be the same");
  if (isSingleWord()) {
    exi_assert(RHS.U.VAL != 0, "Remainder by zero?");
    return APInt(BitWidth, U.VAL % RHS.U.VAL);
  }
  ...
  unsigned rhsBits = RHS.getActiveBits();
  unsigned rhsWords = getNumWords(rhsBits);
  exi_assert(rhsWords, "Performing remainder operation by zero ???");
...
u64 APInt::urem(u64 RHS) const {
  exi_assert(RHS != 0, "Remainder by zero?");
...
void APInt::udivrem(const APInt &LHS, const APInt &RHS,
  ...
  exi_assert(LHS.BitWidth == RHS.BitWidth, "Bit widths must be the same");
  unsigned BitWidth = LHS.BitWidth;

  // First, deal with the easy case
  if (LHS.isSingleWord()) {
    exi_assert(RHS.U.VAL != 0, "Divide by zero?");
    u64 QuotVal = LHS.U.VAL / RHS.U.VAL;
    u64 RemVal = LHS.U.VAL % RHS.U.VAL;
  ...
  unsigned rhsBits  = RHS.getActiveBits();
  unsigned rhsWords = getNumWords(rhsBits);
  exi_assert(rhsWords, "Performing divrem operation by zero ???");
  ...
  if (lhsWords == 1) { // rhsWords is 1 if lhsWords is 1.
    ...
    u64 lhsValue = LHS.U.pVal[0];
    u64 rhsValue = RHS.U.pVal[0];
  ...
  std::memset(Quotient.U.pVal + lhsWords, 0,
              (getNumWords(BitWidth) - lhsWords) * kAPIntWordSize);
  std::memset(Remainder.U.pVal + rhsWords, 0,
              (getNumWords(BitWidth) - rhsWords) * kAPIntWordSize);
}

void APInt::udivrem(const APInt &LHS, u64 RHS, APInt &Quotient,
                    u64 &Remainder) {
  exi_assert(RHS != 0, "Divide by zero?");
  unsigned BitWidth = LHS.BitWidth;
...
void APInt::sdivrem(const APInt &LHS, i64 RHS,
                    APInt &Quotient, i64 &Remainder) {
  u64 R = Remainder;
  if (LHS.isNegative()) {
    if (RHS < 0)
...
void APInt::fromString(unsigned numbits, StrRef str, u8 radix) {
  // Check our assumptions here
  exi_assert(!str.empty(), "Invalid string length");
  exi_assert((radix == 10 || radix == 8 || radix == 16 || radix == 2 ||
              radix == 36),
  ...
  exi_assert((slen <= numbits || radix != 2), "Insufficient bit width");
  exi_assert(((slen-1)*3 <= numbits || radix != 8), "Insufficient bit width");
  exi_assert(((slen-1)*4 <= numbits || radix != 16), "Insufficient bit width");
  exi_assert((((slen-1)*64)/22 <= numbits || radix != 10),
  ...
  for (StrRef::iterator e = str.end(); p != e; ++p) {
    unsigned digit = getDigit(*p, radix);
    exi_assert(digit < radix, "Invalid character in digit string");
...
void APInt::toString(SmallVecImpl<char> &Str, unsigned Radix, bool Signed,
  ...
  if (Radix == 2 || Radix == 8 || Radix == 16) {
    ...
    while (Tmp.getBoolValue()) {
      ...
      exi_assert(Digit < Radix, "divide failed");
      if (InsertSeparators && Pos % Grouping == 0 && Pos > 0)
        Str.push_back('\'');
...
unsigned APInt::tcLSB(const WordType *parts, unsigned n) {
  for (unsigned i = 0; i < n; i++) {
    if (parts[i] != 0) {
      unsigned lsb = exi::countr_zero(parts[i]);
      return lsb + i * kAPIntBitsPerWord;
    }
  }
...
unsigned APInt::tcMSB(const WordType *parts, unsigned n) {
  do {
    ...
    if (parts[n] != 0) {
      static_assert(sizeof(parts[n]) <= sizeof(u64));
      unsigned msb = exi::Log2_64(parts[n]);

      return msb + n * kAPIntBitsPerWord;
    }
  } while (n);
...
APInt::tcExtract(WordType *dst, unsigned dstCount, const WordType *src,
  ...
  unsigned dstParts = (srcBits + kAPIntBitsPerWord - 1) / kAPIntBitsPerWord;
  assert(dstParts <= dstCount);

  unsigned firstSrcPart = srcLSB / kAPIntBitsPerWord;
  tcAssign(dst, src + firstSrcPart, dstParts);

  unsigned shift = srcLSB % kAPIntBitsPerWord;
  tcShiftRight(dst, dstParts, shift);

  // We now have (dstParts * APINT_BITS_PER_WORD - shift) bits from SRC
  // in DST.  If this is less that srcBits, append the rest, else
  // clear the high bits.
  unsigned n = dstParts * kAPIntBitsPerWord - shift;
  if (n < srcBits) {
    WordType mask = lowBitMask (srcBits - n);
    dst[dstParts - 1] |= ((src[firstSrcPart + dstParts] & mask)
                          << n % kAPIntBitsPerWord);
  } else if (n > srcBits) {
    if (srcBits % kAPIntBitsPerWord)
      dst[dstParts - 1] &= lowBitMask (srcBits % kAPIntBitsPerWord);
  }
...
int APInt::tcDivide(WordType *lhs, const WordType *rhs,
  ...
  shiftCount = parts * kAPIntBitsPerWord - shiftCount;
  unsigned n = shiftCount / kAPIntBitsPerWord;
  WordType mask = (WordType) 1 << (shiftCount % kAPIntBitsPerWord);
  ...
  for (;;) {
    ...
    if ((mask >>= 1) == 0) {
      mask = (WordType) 1 << (kAPIntBitsPerWord - 1);
      n--;
    }
  }
...
void APInt::tcShiftLeft(WordType *Dst, unsigned Words, unsigned Count) {
  ...
  unsigned WordShift = std::min(Count / kAPIntBitsPerWord, Words);
  unsigned BitShift = Count % kAPIntBitsPerWord;

  // Fastpath for moving by whole words.
  if (BitShift == 0) {
    std::memmove(Dst + WordShift, Dst, (Words - WordShift) * kAPIntWordSize);
  } else {
    while (Words-- > WordShift) {
      Dst[Words] = Dst[Words - WordShift] << BitShift;
      if (Words > WordShift)
        Dst[Words] |=
          Dst[Words - WordShift - 1] >> (kAPIntBitsPerWord - BitShift);
    }
  }

  // Fill in the remainder with 0s.
  std::memset(Dst, 0, WordShift * kAPIntWordSize);
}
...
void APInt::tcShiftRight(WordType *Dst, unsigned Words, unsigned Count) {
  ...
  // Fastpath for moving by whole words.
  if (BitShift == 0) {
    std::memmove(Dst, Dst + WordShift, WordsToMove * kAPIntWordSize);
  } else {
    for (unsigned i = 0; i != WordsToMove; ++i) {
      Dst[i] = Dst[i + WordShift] >> BitShift;
      if (i + 1 != WordsToMove)
        Dst[i] |= Dst[i + WordShift + 1] << (kAPIntBitsPerWord - BitShift);
    }
  }

  // Fill in the remainder with 0s.
  std::memset(Dst + WordsToMove, 0, WordShift * kAPIntWordSize);
}
...
APInt exi::APIntOps::RoundingUDiv(const APInt &A, const APInt &B,
                                   APInt::Rounding RM) {
  // Currently udivrem always rounds down.
  switch (RM) {
  case APInt::Rounding::kDown:
  case APInt::Rounding::kTowardZero:
    return A.udiv(B);
  case APInt::Rounding::kUp: {
    APInt Quo, Rem;
  ...
  exi_unreachable("Unknown APInt::Rounding enum");
}

APInt exi::APIntOps::RoundingSDiv(const APInt &A, const APInt &B,
                                   APInt::Rounding RM) {
  switch (RM) {
  case APInt::Rounding::kDown:
  case APInt::Rounding::kUp: {
...
Option<APInt>
exi::APIntOps::SolveQuadraticEquationWrap(APInt A, APInt B, APInt C,
                                           unsigned RangeWidth) {
  unsigned CoeffWidth = A.getBitWidth();
  assert(CoeffWidth == B.getBitWidth() && CoeffWidth == C.getBitWidth());
  exi_assert(RangeWidth <= CoeffWidth,
         "Value range width should be less than coefficient width");
  exi_assert(RangeWidth > 1, "Value range bit width should be > 1");

  DEBUG_ONLY(dbgs() << __func__ << ": solving " << A << "x^2 + " << B
                    << "x + " << C << ", rw:" << RangeWidth << '\n');

  // Identify 0 as a (non)solution immediately.
  if (C.sextOrTrunc(RangeWidth).isZero()) {
    DEBUG_ONLY(dbgs() << __func__ << ": zero solution\n");
  ...
  DEBUG_ONLY(dbgs() << __func__ << ": updated coefficients " << A << "x^2 + "
                    << B << "x + " << C << ", rw:" << RangeWidth << '\n');

  APInt D = SqrB - 4*A*C;
  exi_assert(D.isNonNegative(), "Negative discriminant");
  APInt SQ = D.sqrt();
  ...
  exi_assert(X.isNonNegative(), "Solution should be non-negative");

  if (!InexactSQ && Rem.isZero()) {
    DEBUG_ONLY(dbgs() << __func__ << ": solution (root): " << X << '\n');
    return X;
  }

  exi_assert((SQ*SQ).sle(D), "SQ = |_sqrt(D)_|, so SQ*SQ <= D");
  ...
  if (!SignChange) {
    DEBUG_ONLY(dbgs() << __func__ << ": no valid solution\n");
    return std::nullopt;
  }

  X += 1;
  DEBUG_ONLY(dbgs() << __func__ << ": solution (wrap): " << X << '\n');
  return X;
}

Option<unsigned>
exi::APIntOps::GetMostSignificantDifferentBit(const APInt &A, const APInt &B) {
  exi_assert(A.getBitWidth() == B.getBitWidth(), "Must have the same bitwidth");
  if (A == B)
    return std::nullopt;
  return A.getBitWidth() - ((A ^ B).countl_zero() + 1);
}

APInt exi::APIntOps::ScaleBitMask(const APInt &A, unsigned NewBitWidth,
                                   bool MatchAllBits) {
...
void exi::StoreIntToMemory(const APInt &IntVal, u8 *Dst,
                            unsigned StoreBytes) {
  exi_assert((IntVal.getBitWidth()+7)/8 >= StoreBytes, "Integer too small!");
  const u8 *Src = (const u8 *)IntVal.getRawData();
  ...
  if (sys::IsLittleEndianHost) {
    ...
    while (StoreBytes > sizeof(u64)) {
      StoreBytes -= sizeof(u64);
      // May not be aligned so use memcpy.
      memcpy(Dst + StoreBytes, Src, sizeof(u64));
      Src += sizeof(u64);
    }

    memcpy(Dst, Src + sizeof(u64) - StoreBytes, StoreBytes);
  }
}

/// LoadIntFromMemory - Loads the integer stored in the LoadBytes bytes starting
/// from Src into IntVal, which is assumed to be wide enough and to hold zero.
void exi::LoadIntFromMemory(APInt &IntVal, const u8 *Src,
                             unsigned LoadBytes) {
  exi_assert((IntVal.getBitWidth()+7)/8 >= LoadBytes, "Integer too small!");
  u8 *Dst = reinterpret_cast<u8 *>(
                   const_cast<u64 *>(IntVal.getRawData()));
  ...
  if (sys::IsLittleEndianHost)
    ...
    // a word.
    while (LoadBytes > sizeof(u64)) {
      LoadBytes -= sizeof(u64);
      // May not be aligned so use memcpy.
      memcpy(Dst, Src + LoadBytes, sizeof(u64));
      Dst += sizeof(u64);
    }

    memcpy(Dst + sizeof(u64) - LoadBytes, Src, LoadBytes);
  }
}
...
APInt APIntOps::mulhs(const APInt &C1, const APInt &C2) {
  exi_assert(C1.getBitWidth() == C2.getBitWidth(), "Unequal bitwidths");
...
APInt APIntOps::pow(const APInt &X, i64 N) {
  exi_assert(N >= 0, "negative exponents not supported.");
  APInt Acc = APInt(X.getBitWidth(), 1);
  if (N == 0)
    return Acc;
  APInt Base = X;
  i64 RemainingExponent = N;
