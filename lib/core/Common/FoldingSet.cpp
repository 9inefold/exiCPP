...
///
void FoldingSetNodeID::AddString(StrRef Str) {
  ...
  unsigned Pos = 0;
  const unsigned *Base = (const unsigned*) Str.data();
  ...
  if (!((intptr_t)Base & 3)) {
    ...
    if constexpr (sys::IsBigEndianHost) {
      for (Pos += 4; Pos <= Size; Pos += 4) {
        unsigned V = ((unsigned char)Str[Pos - 4] << 24) |
                     ((unsigned char)Str[Pos - 3] << 16) |
                     ((unsigned char)Str[Pos - 2] << 8) |
                      (unsigned char)Str[Pos - 1];
        Bits.push_back(V);
      }
    } else {  // Little-endian host
      for (Pos += 4; Pos <= Size; Pos += 4) {
        unsigned V = ((unsigned char)Str[Pos - 1] << 24) |
                     ((unsigned char)Str[Pos - 2] << 16) |
                     ((unsigned char)Str[Pos - 3] << 8) |
                      (unsigned char)Str[Pos - 4];
        Bits.push_back(V);
      }
    }
  }
  ...
  switch (Pos - Size) {
  case 1: V = (V << 8) | (unsigned char)Str[Size - 3]; [[fallthrough]];
  case 2: V = (V << 8) | (unsigned char)Str[Size - 2]; [[fallthrough]];
  case 3: V = (V << 8) | (unsigned char)Str[Size - 1]; break;
...
FoldingSetNodeIDRef FoldingSetNodeID::Intern(BumpPtrAllocator &Allocator) const {
  ...
  FastUninitCopy(New, Bits.data(), Bits.size_in_bytes());
...
static void **GetBucketPtr(void *NextInBucketPtr) {
  ...
  exi_assert((Ptr & 1), "Not a bucket pointer");
...
FoldingSetBase::FoldingSetBase(unsigned Log2InitSize) {
  exi_assert((Log2InitSize > 5 && Log2InitSize < 32),
...
FoldingSetBase &FoldingSetBase::operator=(FoldingSetBase &&RHS) {
  exi_free(Buckets); // This may be null if the set is in a moved-from state.
  Buckets = RHS.Buckets;
...
void FoldingSetBase::GrowBucketCount(unsigned NewBucketCount,
  ...
  exi_assert((NewBucketCount > NumBuckets),
             "Can't shrink a folding set with GrowBucketCount");
  exi_assert(isPowerOf2_32(NewBucketCount), "Bad bucket count!");
...
void FoldingSetBase::reserve(unsigned EltCount, const FoldingSetInfo &Info) {
  ...
  if(EltCount < capacity())
    return;
  GrowBucketCount(exi::bit_floor(EltCount), Info);
}
