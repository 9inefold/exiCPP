...
inline bool ParseMangling(StrRef A, ExiOptions& Opts, ExtraOptions& ExtraOpts) {
  auto [Mangled, Extra] = A.split('%');
  if (!exi::exi_demangle_options(Opts, Mangled))
    return false;
  if (Extra.empty())
    return true;
  // Handle extra information
  exi_todo("Extra mangling section unimplemented");
  return false;
}
