...
protected:
  ...
  /// faster.
  Node *FindNodeOrInsertPos(const FoldingSetNodeID &ID,
                                     void *&InsertPos,
...
/// than using plain FoldingSetNodeIDs, since the 32-element SmallVec
...
public:
  ...
  unsigned computeStableHash() const {
    return static_cast<unsigned>(rhash_64bits(ArrayRef(
...
class FoldingSetNodeID {
  ...
  /// Use a SmallVec to avoid a heap allocation in the common case.
  SmallVec<unsigned, 32> Bits;
...
public:
  ...
  void AddInteger(unsigned long I) {
    if (sizeof(long) == sizeof(int))
      ...
      exi_unreachable("unexpected sizeof(long)");
  }
...
public:
  ...
  void InsertNode(T* N) {
    ...
    exi_assert(Inserted == N, "Node already inserted!");
  }
};
...
template <class T, class VectorT = SmallVec<T*, 8>>
...
struct FoldingSetTrait<T, std::enable_if_t<std::is_enum<T>::value>> {
  static void Profile(const T &X, FoldingSetNodeID &ID) {
    ID.AddInteger(exi::to_underlying(X));
  }
};
