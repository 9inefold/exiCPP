...
///
/// \file
/// This file defines the DenseMap class, a densely-probed hash table.
///
//===----------------------------------------------------------------===//
...
#include <Common/EpochTracker.hpp>
...
template <typename KeyT, typename ValueT,
          ...
          typename Bucket = exi::H::DenseMapPair<KeyT, ValueT>,
          bool IsConst = false>
...
public:
  ...
  void clear() {
    ...
    if constexpr (std::is_trivially_destructible_v<ValueT>) {
      ...
      exi_assert(NumEntries == 0, "Node count imbalance!");
      (void)NumEntries;
    }
    setNumEntries(0);
    setNumTombstones(0);
  ...
  const ValueT &at(const_arg_type_t<KeyT> Val) const {
    ...
    exi_assert(Iter != this->end(), "DenseMap::at failed due to a missing key");
    return Iter->second;
  }
...
protected:
  ...
  void initEmpty() {
    ...
    exi_assert((getNumBuckets() & (getNumBuckets() - 1)) == 0,
  ...
  void moveFromOldBuckets(BucketT *OldBucketsBegin, BucketT *OldBucketsEnd) {
    ...
    for (BucketT *B = OldBucketsBegin, *E = OldBucketsEnd; B != E; ++B) {
      if (!KeyInfoT::isEqual(B->getFirst(), EmptyKey) &&
        ...
        exi_assert(!FoundVal, "Key already in new map?");
  ...
  void copyFrom(
    ...
    const usize NumBuckets = getNumBuckets();
...
private:
  ...
  BucketT *InsertIntoBucketImpl(const LookupKeyT &Lookup, BucketT *TheBucket) {
    ...
    if (EXI_UNLIKELY(NewNumEntries * 4 >= NumBuckets * 3)) {
      this->grow(NumBuckets * 2);
      LookupBucketFor(Lookup, TheBucket);
      NumBuckets = getNumBuckets();
    } else if (EXI_UNLIKELY(NumBuckets -
  ...
  template <typename LookupKeyT> BucketT *doFind(const LookupKeyT &Val) {
    ...
    while (true) {
      ...
      if (EXI_LIKELY(KeyInfoT::isEqual(Val, Bucket->getFirst())))
        return Bucket;
      if (EXI_LIKELY(KeyInfoT::isEqual(Bucket->getFirst(), EmptyKey)))
        return nullptr;
  ...
  bool LookupBucketFor(const LookupKeyT &Val, BucketT *&FoundBucket) {
    ...
    const KeyT EmptyKey = getEmptyKey();
    const KeyT TombstoneKey = getTombstoneKey();
    exi_assert(!KeyInfoT::isEqual(Val, EmptyKey) &&
    ...
    while (true) {
      ...
      if (EXI_LIKELY(KeyInfoT::isEqual(Val, ThisBucket->getFirst()))) {
      ...
      if (EXI_LIKELY(KeyInfoT::isEqual(ThisBucket->getFirst(), EmptyKey))) {
...
public:
  ...
  /// are not included.
  usize getMemorySize() const { return getNumBuckets() * sizeof(BucketT); }
};
...
template <typename KeyT, typename ValueT,
          typename KeyInfoT = DenseMapInfo<KeyT>,
          typename BucketT = exi::H::DenseMapPair<KeyT, ValueT>>
...
public:
  explicit SmallDenseMap(unsigned NumInitBuckets = 0) {
    if (NumInitBuckets > InlineBuckets)
      NumInitBuckets = exi::bit_ceil(NumInitBuckets);
    init(NumInitBuckets);
  }
  ...
  void grow(unsigned AtLeast) {
    ...
    if (Small) {
      ...
      for (BucketT *P = getBuckets(), *E = P + InlineBuckets; P != E; ++P) {
        if (!KeyInfoT::isEqual(P->getFirst(), EmptyKey) &&
            !KeyInfoT::isEqual(P->getFirst(), TombstoneKey)) {
          exi_assert(usize(TmpEnd - TmpBegin) < InlineBuckets,
...
private:
  ...
  void setNumEntries(unsigned Num) {
    ...
    exi_assert(Num < (1U << 31), "Cannot support more than 1<<31 entries");
    NumEntries = Num;
  }
...
public:
  ...
  DenseMapIterator(pointer Pos, pointer E, const DebugEpochBase &Epoch,
    ...
    exi_invariant(isHandleInSync(), "invalid construction!");
  ...
  friend bool operator==(const DenseMapIterator &LHS,
    ...
    exi_invariant((!LHS.Ptr || LHS.isHandleInSync()), "handle not in sync!");
    exi_invariant((!RHS.Ptr || RHS.isHandleInSync()), "handle not in sync!");
  ...
  inline DenseMapIterator &operator++() { // Preincrement
    exi_invariant(isHandleInSync(), "invalid iterator access!");
    exi_invariant(Ptr != End, "incrementing end() iterator");
    if (shouldReverseIterate<KeyT>()) {
      --Ptr;
      RetreatPastEmptyBuckets();
      return *this;
...
inline usize capacity_in_bytes(const DenseMap<KeyT, ValueT, KeyInfoT> &X) {
