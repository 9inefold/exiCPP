...
protected:
  ...
  /// of the string.
  unsigned LookupBucketFor(StrRef Key) {
    return LookupBucketFor(Key, hash(Key));
  }

  /// Overload that explicitly takes precomputed hash(Key).
  unsigned LookupBucketFor(StrRef Key, u32 FullHashValue);

  /// FindKey - Look up the bucket that contains the specified key. If it exists
  /// in the map, return the bucket number of the key.  Otherwise return -1.
  /// This does not modify the map.
  int FindKey(StrRef Key) const { return FindKey(Key, hash(Key)); }

  /// Overload that explicitly takes precomputed hash(Key).
  int FindKey(StrRef Key, u32 FullHashValue) const;
  ...
  StringMapEntryBase *RemoveKey(StrRef Key);
...
public:
  ...
  StringMap(std::initializer_list<std::pair<StrRef, ValueTy>> List)
  ...
  ValueTy lookup(StrRef Key) const {
  ...
  /// entry exists.
  const ValueTy &at(StrRef Val) const {
    auto Iter = this->find(std::move(Val));
    exi_assert(Iter != this->end(), "StringMap::at failed due to a missing key");
    return Iter->second;
  }

  /// Lookup the ValueTy for the \p Key, or create a default constructed value
  /// if the key is not in the map.
  ValueTy &operator[](StrRef Key) { return try_emplace(Key).first->second; }
  ...
  bool operator==(const StringMap &RHS) const {
    ...
    for (const auto &KeyValue : *this) {
      ...
      if constexpr (!std::is_same_v<ValueTy, nullopt_t>) {
  ...
  /// element is inserted
  void insert(std::initializer_list<std::pair<StrRef, ValueTy>> List) {
  ...
  template <typename V>
  std::pair<iterator, bool> insert_or_assign(StrRef Key, V &&Val) {
...
class StringMapKeyIterator
    : public iterator_adaptor_base<StringMapKeyIterator<ValueTy>,
                                   ...
                                   std::forward_iterator_tag, StrRef> {
...
public:
  ...
  StrRef operator*() const { return this->wrapped()->getKey(); }
};
