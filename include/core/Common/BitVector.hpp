...
template <typename BitVectorT> class const_set_bits_iterator_impl {
  ...
  void advance() {
    exi_assert(Current != -1, "Trying to advance past end.");
...
public:
  ...
  bool back() const {
    exi_assert(!empty(), "Getting last element of empty vector.");
    return (*this)[size() - 1];
  }
  ...
  void pop_back() {
    exi_assert(!empty(), "Empty vector has no element to pop.");
    resize(size() - 1);
  }
  ...
  //===--------------------------------------------------------------------===//
  //
  // These methods all operate on arrays of u32, each holding 32 bits. The
  ...
  void setBitsInMask(const u32 *Mask, unsigned MaskWords = ~0u) {
    applyMask<true, false>(Mask, MaskWords);
  }

  /// clearBitsInMask - Clear any bits in this vector that are set in Mask.
  /// Don't resize. This computes "*this &= ~Mask".
  void clearBitsInMask(const u32 *Mask, unsigned MaskWords = ~0u) {
    applyMask<false, false>(Mask, MaskWords);
  }

  /// setBitsNotInMask - Add a bit to this vector for every '0' bit in Mask.
  /// Don't resize.  This computes "*this |= ~Mask".
  void setBitsNotInMask(const u32 *Mask, unsigned MaskWords = ~0u) {
    applyMask<true, true>(Mask, MaskWords);
  }

  /// clearBitsNotInMask - Clear a bit in this vector for every '0' bit in Mask.
  /// Don't resize.  This computes "*this &= Mask".
  void clearBitsNotInMask(const u32 *Mask, unsigned MaskWords = ~0u) {
...
private:
  ...
  void wordShl(u32 Count) {
    if (Count == 0)
      return;

    u32 NumWords = Bits.size();
...
inline void swap(exi::BitVector &LHS, exi::BitVector &RHS) { LHS.swap(RHS); }
} // end namespace std
