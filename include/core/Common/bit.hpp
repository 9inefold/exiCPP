...
#if EXI_HAS_BUILTIN(__builtin_bit_cast)
...
/// This implementation of bit_cast is different from the C++20 one, as
...
[[nodiscard]] inline EXI_BIT_CAST_CONSTEXPR
...
#else
    uint64_t Hi = exi::byteswap<uint32_t>(UV);
    uint32_t Lo = exi::byteswap<uint32_t>(UV >> 32);
    return (Hi << 32) | Lo;
#endif
  } else {
...
template <typename T> [[nodiscard]] int countr_zero(T Val) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return exi::detail::TrailingZerosCounter<T, sizeof(T)>::count(Val);
}
...
template <typename T> [[nodiscard]] int countl_zero(T Val) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return exi::detail::LeadingZerosCounter<T, sizeof(T)>::count(Val);
}
...
template <typename T> [[nodiscard]] int countl_one(T Value) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return exi::countl_zero<T>(~Value);
}
...
template <typename T> [[nodiscard]] int countr_one(T Value) {
  static_assert(std::is_unsigned_v<T>,
                "Only unsigned integral types are allowed.");
  return exi::countr_zero<T>(~Value);
}
...
template <typename T> [[nodiscard]] T bit_floor(T Value) {
  ...
  if (!Value)
    return 0;
  return T(1) << (exi::bit_width(Value) - 1);
}
