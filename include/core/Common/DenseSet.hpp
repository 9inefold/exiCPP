...
///
///   DenseMap<ValueT, H::DenseSetEmpty, ValueInfoT,
///            H::DenseSetPair<ValueT>>
///
...
public:
  ...
  usize getMemorySize() const { return TheMap.getMemorySize(); }

  /// Grow the DenseSet so that it has at least Size buckets. Will not shrink
  /// the Size of the set.
  void resize(usize Size) { TheMap.resize(Size); }

  /// Grow the DenseSet so that it can contain at least \p NumEntries items
  /// before resizing again.
  void reserve(usize Size) { TheMap.reserve(Size); }
  ...
  std::pair<iterator, bool> insert_as(const ValueT &V,
    ...
    return TheMap.insert_as({V, H::DenseSetEmpty()}, LookupKey);
  }
  template <typename LookupKeyT>
  std::pair<iterator, bool> insert_as(ValueT &&V, const LookupKeyT &LookupKey) {
    return TheMap.insert_as({std::move(V), H::DenseSetEmpty()}, LookupKey);
  }
...
class DenseSet : public H::DenseSetImpl<
                     ValueT,
                     DenseMap<ValueT, H::DenseSetEmpty, ValueInfoT,
                              H::DenseSetPair<ValueT>>,
                     ValueInfoT> {
  using BaseT =
      H::DenseSetImpl<ValueT,
                           DenseMap<ValueT, H::DenseSetEmpty, ValueInfoT,
                                    H::DenseSetPair<ValueT>>,
                           ValueInfoT>;
...
class SmallDenseSet
    : public H::DenseSetImpl<
          ValueT,
          SmallDenseMap<ValueT, H::DenseSetEmpty, InlineBuckets,
                        ValueInfoT, H::DenseSetPair<ValueT>>,
          ValueInfoT> {
  using BaseT = H::DenseSetImpl<
      ValueT,
      SmallDenseMap<ValueT, H::DenseSetEmpty, InlineBuckets, ValueInfoT,
                    H::DenseSetPair<ValueT>>,
      ValueInfoT>;
