...
/// "unsigned", "unsigned long" or "u64", but also allows non-byte-width
...
public:
  ...
  /// Byte size of a word.
  static constexpr unsigned kAPIntWordSize = sizeof(WordType);

  /// Bits in a word.
  static constexpr unsigned kAPIntBitsPerWord = bitsizeof_v<WordType>;
  ...
  static constexpr WordType kWordTypeMax = ~WordType(0);
  ...
  /// (i.e. as an i64) and the appropriate sign extension to the bit width
  ...
  APInt(unsigned numBits, u64 val, bool isSigned = false,
        bool implicitTrunc = false)
      : BitWidth(numBits) {
    if (!implicitTrunc) {
      if (isSigned) {
        if (BitWidth == 0) {
          exi_assert((val == 0 || val == u64(-1)),
                 "Value must be 0 or -1 for signed 0-bit APInt");
        } else {
          exi_assert(exi::isIntN(BitWidth, val),
                 "Value is not an N-bit signed value");
        }
      } else {
        if (BitWidth == 0) {
          exi_assert(val == 0, "Value must be zero for unsigned 0-bit APInt");
        } else {
          exi_assert(exi::isUIntN(BitWidth, val),
  ...
  APInt(unsigned numBits, ArrayRef<u64> bigVal);

  /// Equivalent to APInt(numBits, ArrayRef<u64>(bigVal, numWords)), but
  /// deprecated because this constructor is prone to ambiguity with the
  /// APInt(unsigned, u64, bool) constructor.
  ///
  /// If this overload is ever deleted, care should be taken to prevent calls
  /// from being incorrectly captured by the APInt(unsigned, u64, bool)
  /// constructor.
  APInt(unsigned numBits, unsigned numWords, const u64 bigVal[]);
  ...
  APInt(unsigned numBits, StrRef str, u8 radix);
  ...
  static APInt getAllOnes(unsigned numBits) {
    return APInt(numBits, kWordTypeMax, true);
  }
  ...
  bool isSingleWord() const { return BitWidth <= kAPIntBitsPerWord; }
  ...
  u64 getLimitedValue(u64 Limit = UINT64_MAX) const {
  ...
  APInt &operator=(u64 RHS) {
    ...
    U.pVal[0] = RHS;
    memset(U.pVal + 1, 0, (getNumWords() - 1) * kAPIntWordSize);
    return *this;
  }
  ...
  static void udivrem(const APInt &LHS, const APInt &RHS, APInt &Quotient,
                      APInt &Remainder);
  static void udivrem(const APInt &LHS, u64 RHS, APInt &Quotient,
                      u64 &Remainder);

  static void sdivrem(const APInt &LHS, const APInt &RHS, APInt &Quotient,
                      APInt &Remainder);
  static void sdivrem(const APInt &LHS, i64 RHS, APInt &Quotient,
                      i64 &Remainder);
  ...
  bool operator[](unsigned bitPosition) const {
    exi_assert(bitPosition < getBitWidth(), "Bit position out of bounds!");
  ...
  /// Equality operator.
  ///
  /// Compares this APInt with a u64 for the validity of the equality
  /// relationship.
  ///
  ...
  /// Inequality operator.
  ///
  /// Compares this APInt with a u64 for the validity of the inequality
  /// relationship.
  ///
  /// \returns true if *this != Val
  bool operator!=(u64 Val) const { return !((*this) == Val); }
  ...
  ///
  /// \returns true if *this <= RHS when considered unsigned.
  bool ule(u64 RHS) const { return !ugt(RHS); }
  ...
  ///
  /// \returns true if *this <= RHS when considered signed.
  bool sle(u64 RHS) const { return !sgt(RHS); }
  ...
  ///
  /// \returns true if *this >= RHS when considered unsigned.
  bool uge(u64 RHS) const { return !ult(RHS); }
  ...
  ///
  /// \returns true if *this >= RHS when considered signed.
  bool sge(i64 RHS) const { return !slt(RHS); }
  ...
  void setAllBits() {
    if (isSingleWord())
      ...
      memset(U.pVal, -1, getNumWords() * kAPIntWordSize);
  ...
  void setBitsWithWrap(unsigned loBit, unsigned hiBit) {
    exi_assert(hiBit <= BitWidth, "hiBit out of range");
    exi_assert(loBit <= BitWidth, "loBit out of range");
    if (loBit < hiBit) {
      setBits(loBit, hiBit);
      return;
    }
  ...
  void setBits(unsigned loBit, unsigned hiBit) {
    exi_assert(hiBit <= BitWidth, "hiBit out of range");
    exi_assert(loBit <= BitWidth, "loBit out of range");
    exi_assert(loBit <= hiBit, "loBit greater than hiBit");
    if (loBit == hiBit)
      return;
  ...
  void clearLowBits(unsigned loBits) {
    exi_assert(loBits <= BitWidth, "More bits than bitwidth");
  ...
  void clearHighBits(unsigned hiBits) {
    exi_assert(hiBits <= BitWidth, "More bits than bitwidth");
  ...
  void insertBits(u64 SubBits, unsigned bitPosition, unsigned numBits);

  /// Return an APInt with the extracted bits [bitPosition,bitPosition+numBits).
  APInt extractBits(unsigned numBits, unsigned bitPosition) const;
  u64 extractBitsAsZExtValue(unsigned numBits, unsigned bitPosition) const;
  ...
  /// Get the number of words.
  ///
  /// *NOTE* Here one word's bitwidth equals to that of u64.
  ///
  ...
  /// This method attempts to return the value of this APInt as a zero extended
  /// u64. The bitwidth must be <= 64 or the value must fit within a
  /// u64. Otherwise no value is returned.
  Option<u64> tryZExtValue() const {
    return (getActiveBits() <= 64) ? Option<u64>(getZExtValue())
                                   : std::nullopt;
  };
  ...
  ///
  /// This method attempts to return the value of this APInt as a sign extended
  /// i64. The bitwidth must be <= 64 or the value must fit within an
  /// i64. Otherwise no value is returned.
  Option<i64> trySExtValue() const {
    return (getSignificantBits() <= 64) ? Option<i64>(getSExtValue())
                                        : std::nullopt;
  };
  ...
  static unsigned getBitsNeeded(StrRef str, u8 radix);
  ...
  static unsigned getSufficientBitsNeeded(StrRef Str, u8 Radix);
  ...
  /// digits.
  void toString(SmallVecImpl<char> &Str, unsigned Radix, bool Signed,
                bool formatAsCLiteral = false, bool UpperCase = true,
                bool InsertSeparators = false) const;

  /// Considers the APInt to be unsigned and converts it into a string in the
  /// radix given. The radix can be 2, 8, 10 16, or 36.
  void toStringUnsigned(SmallVecImpl<char> &Str, unsigned Radix = 10) const {
    toString(Str, Radix, false, false);
  }

  /// Considers the APInt to be signed and converts it into a string in the
  /// radix given. The radix can be 2, 8, 10, 16, or 36.
  void toStringSigned(SmallVecImpl<char> &Str, unsigned Radix = 10) const {
  ...
  double bitsToDouble() const { return std::bit_cast<double>(getWord(0)); }

#if HAS_IEE754_FLOAT128
  float128 bitsToQuad() const {
    __uint128_t ul = ((__uint128_t)U.pVal[1] << 64) + U.pVal[0];
    return std::bit_cast<float128>(ul);
  }
#endif
  ...
  float bitsToFloat() const {
    return std::bit_cast<float>(static_cast<u32>(getWord(0)));
  }
  ...
  static APInt doubleToBits(double V) {
    return APInt(sizeof(double) * CHAR_BIT, std::bit_cast<u64>(V));
  }
  ...
  static APInt floatToBits(float V) {
    return APInt(sizeof(float) * CHAR_BIT, std::bit_cast<u32>(V));
  }
...
private:
  ...
  union {
    u64 VAL;   ///< Used to store the <= 64 bits integer value.
    u64 *pVal; ///< Used to store the >64 bits integer value.
  } U;
  ...
  /// is not public.
  APInt(u64 *val, unsigned bits) : BitWidth(bits) { U.pVal = val; }

  /// Determine which word a bit is in.
  ///
  /// \returns the word position for the specified bit position.
  static unsigned whichWord(unsigned bitPosition) {
    return bitPosition / kAPIntBitsPerWord;
  }
  ...
  ///
  /// \returns a u64 with only bit at "whichBit(bitPosition)" set
  /// This method generates and returns a u64 (word) mask for a single
  /// bit at a specific bit position. This is used to mask the bit in the
  /// corresponding word.
  static u64 maskBit(unsigned bitPosition) {
  ...
  APInt &clearUnusedBits() {
    ...
    unsigned WordBits = ((BitWidth - 1) % kAPIntBitsPerWord) + 1;
  ...
  u64 getWord(unsigned bitPosition) const {
  ...
  ///
  /// StrRef::getAsInteger is superficially similar but (1) does
  /// not assume that the string is well-formed and (2) grows the
  /// result to hold the input.
  void fromString(unsigned numBits, StrRef str, u8 radix);
  ...
  void initFromArray(ArrayRef<u64> array);
  ...
  unsigned countLeadingOnesSlowCase() const EXI_READONLY;

  /// out-of-line slow case for countTrailingZeros.
  unsigned countTrailingZerosSlowCase() const EXI_READONLY;

  /// out-of-line slow case for countTrailingOnes
  unsigned countTrailingOnesSlowCase() const EXI_READONLY;

  /// out-of-line slow case for countPopulation
  unsigned countPopulationSlowCase() const EXI_READONLY;

  /// out-of-line slow case for intersects.
  bool intersectsSlowCase(const APInt &RHS) const EXI_READONLY;

  /// out-of-line slow case for isSubsetOf.
  bool isSubsetOfSlowCase(const APInt &RHS) const EXI_READONLY;
  ...
  int compare(const APInt &RHS) const EXI_READONLY;

  /// Signed comparison. Returns -1, 0, or 1 if this APInt is less than, equal
  /// to, or greater than RHS.
  int compareSigned(const APInt &RHS) const EXI_READONLY;

  /// @}
};

inline bool operator==(u64 V1, const APInt &V2) { return V2 == V1; }
...
inline APInt operator&(APInt a, u64 RHS) {
  a &= RHS;
  return a;
}

inline APInt operator&(u64 LHS, APInt b) {
  b &= LHS;
  return b;
}
...
/// coefficients.
Option<APInt> SolveQuadraticEquationWrap(APInt A, APInt B, APInt C,
                                                unsigned RangeWidth);

/// Compare two values, and if they are different, return the position of the
/// most significant bit that is different in the values.
Option<unsigned> GetMostSignificantDifferentBit(const APInt &A,
                                                       const APInt &B);
...
void StoreIntToMemory(const APInt &IntVal, u8 *Dst, unsigned StoreBytes);
