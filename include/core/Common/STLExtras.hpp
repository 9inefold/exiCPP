...
struct TypesAreDistinct
    : std::integral_constant<bool, H::TypesAreDistinct<Ts...>::value> {};
...
using TypeAtIndex = TypePackElement<I, Ts...>;
...
public:
  ...
  template < // clang-format off
    typename FnPtrOrRef,
    std::enable_if_t<
      !std::is_same_v<std::remove_cvref_t<FnPtrOrRef>, Callable>, int
    > = 0
  > // clang-format on
...
template <typename ContainerTy> [[nodiscard]] auto reverse(ContainerTy &&C) {
  if constexpr (H::HasFreeFunctionRBegin<ContainerTy>)
...
public:
  ...
  decltype(auto) operator*() const {
    exi_assert(BaseT::wrapped() != End, "Cannot dereference end iterator!");
...
using filter_iterator = filter_iterator_impl<
    ...
    typename H::fwd_or_bidi_tag<WrappedIteratorT>::type>;
...
iterator_range<filter_iterator<H::IterOfRange<RangeT>, PredicateT>>
make_filter_range(RangeT &&Range, PredicateT Pred) {
  using FilterIteratorT =
      filter_iterator<H::IterOfRange<RangeT>, PredicateT>;
  return make_range(
...
#if EXI_INVARIANTS
    exi_assert(!IsEarlyIncremented, "Cannot dereference twice!");
...
#if EXI_INVARIANTS
    exi_assert(IsEarlyIncremented, "Cannot increment before dereferencing!");
...
#if EXI_INVARIANTS
    exi_assert(!LHS.IsEarlyIncremented, "Cannot compare after dereferencing!");
#endif
...
iterator_range<early_inc_iterator_impl<H::IterOfRange<RangeT>>>
make_early_inc_range(RangeT &&Range) {
  using EarlyIncIteratorT =
      early_inc_iterator_impl<H::IterOfRange<RangeT>>;
...
template <typename T, typename U, typename... Args>
H::zippy<H::zip_first, T, U, Args...> zip_first(T &&t, U &&u,
                                                          Args &&...args) {
  exi_assert(range_size(t) <= std::min({range_size(u), range_size(args)...}),
...
H::zip_longest_range<T, U, Args...> zip_longest(T &&t, U &&u,
                                                     Args &&... args) {
  return H::zip_longest_range<T, U, Args...>(
      std::forward<T>(t), std::forward<U>(u), std::forward<Args>(args)...);
}
...
class concat_iterator
  ...
  template <size_t... Ns> void increment(std::index_sequence<Ns...>) {
    ...
    exi_unreachable("Attempted to increment an end concat iterator!");
  }
  ...
  template <size_t... Ns> reference_type get(std::index_sequence<Ns...>) const {
    ...
    exi_unreachable("Attempted to get a pointer from an end concat iterator!");
  }
...
[[nodiscard]] H::concat_range<ValueT, RangeTs...>
concat(RangeTs &&...Ranges) {
  static_assert(sizeof...(RangeTs) > 1,
                "Need more than one range to concatenate!");
  return H::concat_range<ValueT, RangeTs...>(
...
class indexed_accessor_iterator
    : public exi::iterator_facade_base<DerivedT,
                                        std::random_access_iterator_tag, T,
                                        std::ptrdiff_t, PointerT, ReferenceT> {
public:
  ptrdiff_t operator-(const indexed_accessor_iterator &rhs) const {
    exi_assert(base == rhs.base, "incompatible iterators");
...
public:
  ...
  ReferenceT operator[](size_t Index) const {
    exi_assert(Index < size(), "invalid index for value range");
    return DerivedT::dereference_iterator(base, static_cast<ptrdiff_t>(Index));
  }
  ReferenceT front() const {
    exi_assert(!empty(), "expected non-empty range");
    return (*this)[0];
  }
  ...
  DerivedT slice(size_t n, size_t m) const {
    exi_assert(n + m <= size(), "invalid size specifiers");
  ...
  template <typename RangeT, typename = std::enable_if_t<
...
class indexed_accessor_range
    : public H::indexed_accessor_range_base<
          DerivedT, std::pair<BaseT, ptrdiff_t>, T, PointerT, ReferenceT> {
public:
  indexed_accessor_range(BaseT base, ptrdiff_t startIndex, ptrdiff_t count)
      : H::indexed_accessor_range_base<
            DerivedT, std::pair<BaseT, ptrdiff_t>, T, PointerT, ReferenceT>(
            std::make_pair(base, startIndex), count) {}
  using H::indexed_accessor_range_base<
  ...
  /// See `H::indexed_accessor_range_base` for details.
...
template <typename ContainerTy> auto make_first_range(ContainerTy &&c) {
  ...
  return exi::map_range(std::forward<ContainerTy>(c),
                         [](EltTy elt) -> typename H::first_or_second_type<
...
template <typename ContainerTy> auto make_second_range(ContainerTy &&c) {
  using EltTy = decltype((*std::begin(c)));
  return exi::map_range(
      std::forward<ContainerTy>(c),
      [](EltTy elt) ->
      typename H::first_or_second_type<EltTy,
...
struct Visitor<HeadT, TailTs...> :
 std::remove_cvref_t<HeadT>, Visitor<TailTs...> {
  explicit constexpr Visitor(HeadT &&Head, TailTs &&...Tail)
      : std::remove_cvref_t<HeadT>(std::forward<HeadT>(Head)),
        Visitor<TailTs...>(std::forward<TailTs>(Tail)...) {}
  using std::remove_cvref_t<HeadT>::operator();
  using Visitor<TailTs...>::operator();
};

template <typename HeadT> struct Visitor<HeadT> : std::remove_cvref_t<HeadT> {
...
constexpr decltype(auto) makeVisitor(CallableTs &&...Callables) {
  return H::Visitor<CallableTs...>(std::forward<CallableTs>(Callables)...);
}
...
inline void presortShuffle(IteratorTy Start, IteratorTy End) {
  ...
  exi::shuffle(Start, End, Generator);
}
...
template <typename Container> inline void sort(Container &&C) {
  exi::sort(adl_begin(C), adl_end(C));
}
...
inline void sort(Container &&C, Compare Comp) {
  exi::sort(adl_begin(C), adl_end(C), Comp);
}
...
/// preference. Unlike `exi::size`, this function does *not* guarantee O(1)
/// running time, and is intended to be used in generic code that does not know
/// the exact range type.
template <typename R> constexpr size_t range_size(R &&Range) {
  if constexpr (H::HasFreeFunctionSize<R>)
...
bool is_contained(R &&Range, const E &Element) {
  if constexpr (H::HasMemberContains<R, E>)
    return Range.contains(Element);
  else if constexpr (H::HasMemberFind<R, E>)
...
template <typename ForwardIterator, typename UnaryFunctor,
          ...
          typename = std::enable_if_t<
              !std::is_constructible_v<StrRef, UnaryFunctor> &&
              !std::is_constructible_v<StrRef, NullaryFunctor>>>
...
template <typename Container, typename UnaryFunctor, typename StreamT,
          typename T = H::ValueOfRange<Container>>
inline void interleave(const Container &c, StreamT &os, UnaryFunctor each_fn,
                       const StrRef &separator) {
...
struct index_iterator
    : exi::iterator_facade_base<index_iterator,
...
public:
  ...
  H::index_iterator begin() const { return {Begin}; }
  H::index_iterator end() const { return {End}; }
};
...
auto enumerate(FirstRange &&First, RestRanges &&...Rest) {
  if constexpr (sizeof...(Rest) != 0) {
    ...
    exi_assert(all_equal(sizes), "Ranges have different length");
#endif
  }
  using enumerator = H::zippy<H::zip_enumerator, H::index_stream,
                                   FirstRange, RestRanges...>;
  return enumerator(H::index_stream{}, std::forward<FirstRange>(First),
...
bool all_of_zip(ArgsAndPredicate &&...argsAndPredicate) {
  return H::all_of_zip_predicate_last(
...
template <typename T>
constexpr bool is_incomplete_v = !is_detected<H::has_sizeof, T>::value;

} // namespace exi

namespace std {
template <typename... Refs>
struct tuple_size<exi::H::enumerator_result<Refs...>>
    : std::integral_constant<std::size_t, sizeof...(Refs)> {};

template <std::size_t I, typename... Refs>
struct tuple_element<I, exi::H::enumerator_result<Refs...>>
    : std::tuple_element<I, std::tuple<Refs...>> {};

template <std::size_t I, typename... Refs>
struct tuple_element<I, const exi::H::enumerator_result<Refs...>>
    : std::tuple_element<I, std::tuple<Refs...>> {};
