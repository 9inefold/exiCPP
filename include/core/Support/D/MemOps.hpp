...
EXI_INLINE exi_mem_constexpr void FastInitCopy(T* Dst, const T* Src, usize N) {
...
template <bool PotentiallyOverlapping = false, typename T>
EXI_INLINE exi_mem_constexpr void FastUninitCopy(T* Dst, const T* Src, usize N) {
  if constexpr (std::is_trivially_copyable_v<T>)
    H::FastTrivialCopy<PotentiallyOverlapping>(Dst, Src, N);
  else
...
EXI_INLINE exi_mem_constexpr void FastCopy(T* Dst, const T* Src, usize N) {
  if constexpr (std::is_trivially_copyable_v<T>)
    H::FastTrivialCopy<PotentiallyOverlapping>(Dst, Src, N);
  else
