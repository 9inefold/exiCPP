...
template <typename HasherT, exi::endianness Endianness> class HashBuilder;
class raw_ostream;
class StrRef;
...
public:
  ...
  Option<unsigned> getMinor() const {
    if (!HasMinor)
      return std::nullopt;
    return Minor;
  }
  ...
  Option<unsigned> getSubminor() const {
    if (!HasSubminor)
      return std::nullopt;
    return Subminor;
  }
  ...
  Option<unsigned> getBuild() const {
    if (!HasBuild)
      return std::nullopt;
    return Build;
  }
  ...
  template <typename HasherT, exi::endianness Endianness>
