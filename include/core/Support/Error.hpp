...
public:
  ...
  Error(Box<ErrorInfoBase> Payload) {
...
private:
  ...
  EXI_INLINE void assertIsChecked() {
#if EXI_INVARIANTS
    if (EXI_UNLIKELY(!getChecked() || getPtr())) {
      fatalUncheckedError();
    }
#endif
  }
  ...
  Box<ErrorInfoBase> takePayload() {
    Box<ErrorInfoBase> Tmp(getPtr());
    setPtr(nullptr);
    setChecked(true);
    return Tmp;
  }
...
private:
  ErrorList(Box<ErrorInfoBase> Payload1,
            Box<ErrorInfoBase> Payload2) {
    exi_assert(!Payload1->isA<ErrorList>() && !Payload2->isA<ErrorList>(),
  ...
  static Error join(Error E1, Error E2) {
    ...
    return Error(Box<ErrorList>(
        new ErrorList(E1.takePayload(), E2.takePayload())));
  }

  std::vector<Box<ErrorInfoBase>> Payloads;
};
...
template <class T> class [[nodiscard]] Expected {
  ...
  using error_type = Box<ErrorInfoBase>;
...
#endif
  {
    exi_assert(Err, "Cannot create Expected<T> from Error success value.");
...
private:
  ...
  storage_type *getStorage() {
    exi_assert(!HasError, "Cannot get value when an error exists!");
  ...
  error_type *getErrorStorage() {
    exi_assert(HasError, "Cannot get error when a value exists!");
...
public:
  ...
  template <typename HandlerT>
  static Error apply(HandlerT &&H, Box<ErrorInfoBase> E) {
    exi_assert(appliesTo(*E), "Applying incorrect handler");
...
/// Specialization for functions of the form 'Error (Box<ErrT>)'.
template <typename ErrT>
class ErrorHandlerTraits<Error (&)(Box<ErrT>)> {
public:
  static bool appliesTo(const ErrorInfoBase &E) {
    return E.template isA<ErrT>();
  }
  ...
  static Error apply(HandlerT &&H, Box<ErrorInfoBase> E) {
    exi_assert(appliesTo(*E), "Applying incorrect handler");
    Box<ErrT> SubE(static_cast<ErrT *>(E.release()));
    return H(std::move(SubE));
  }
};

/// Specialization for functions of the form 'void (Box<ErrT>)'.
template <typename ErrT>
class ErrorHandlerTraits<void (&)(Box<ErrT>)> {
public:
  static bool appliesTo(const ErrorInfoBase &E) {
    return E.template isA<ErrT>();
  }
...
/// 'RetT (Box<ErrT>)'.
template <typename C, typename RetT, typename ErrT>
class ErrorHandlerTraits<RetT (C::*)(Box<ErrT>)>
    : public ErrorHandlerTraits<RetT (&)(Box<ErrT>)> {};

/// Specialization for member functions of the form
/// 'RetT (Box<ErrT>) const'.
template <typename C, typename RetT, typename ErrT>
class ErrorHandlerTraits<RetT (C::*)(Box<ErrT>) const>
    : public ErrorHandlerTraits<RetT (&)(Box<ErrT>)> {};

inline Error handleErrorImpl(Box<ErrorInfoBase> Payload) {
  return Error(std::move(Payload));
}

template <typename HandlerT, typename... HandlerTs>
Error handleErrorImpl(Box<ErrorInfoBase> Payload,
...
Error handleErrors(Error E, HandlerTs &&... Hs) {
  ...
  Box<ErrorInfoBase> Payload = E.takePayload();
...
/// errors after running the handlers, or exi_unreachable is called).
template <typename... HandlerTs>
...
/// If E is an error, exi_unreachable will be called.
...
/// might be more clearly refactored to return an Option<T>.
...
///   @code{.cpp}
///   Expected<int> foo() {
///      return exi::make_error<PDBError>(pdb_error_code::dia_failed_loading,
...
public:
  ...
  StringError(String &&S, std::error_code EC, bool PrintMsgOnly);
...
inline Error createStringError(std::error_code EC, const char *S) {
  return createStringError(String(S), EC);
}
...
class FileError final : public ErrorInfo<FileError> {
  ...
  friend Error createFileError(const Twine &, usize, Error);

public:
  void log(raw_ostream &OS) const override {
    exi_assert(Err, "Trying to log after takeError().");
    OS << "'" << FileName << "': ";
    if (Line)
      OS << "line " << *Line << ": ";
    Err->log(OS);
  ...
  String messageWithoutFileInfo() const {
    String Msg;
    raw_string_ostream OS(Msg);
    Err->log(OS);
    return Msg;
  ...
  StrRef getFileName() const { return FileName; }
...
private:
  FileError(const Twine &F, Option<usize> LineNum,
            Box<ErrorInfoBase> E) {
    exi_assert(E, "Cannot create FileError from Error success value.");
    FileName = F.str();
    Err = std::move(E);
    Line = std::move(LineNum);
  }

  static Error build(const Twine &F, Option<usize> Line, Error E) {
    Box<ErrorInfoBase> Payload;
    handleAllErrors(std::move(E),
                    [&](Box<ErrorInfoBase> EIB) -> Error {
                      Payload = std::move(EIB);
                      return Error::success();
                    });
    return Error(
        Box<FileError>(new FileError(F, Line, std::move(Payload))));
  }

  String FileName;
  Option<usize> Line;
  Box<ErrorInfoBase> Err;
};

/// Concatenate a source file path and/or name with an Error. The resulting
/// Error is unchecked.
inline Error createFileError(const Twine &F, Error E) {
  return FileError::build(F, Option<usize>(), std::move(E));
}

/// Concatenate a source file path and/or name with line number and an Error.
/// The resulting Error is unchecked.
inline Error createFileError(const Twine &F, usize Line, Error E) {
  return FileError::build(F, Option<usize>(Line), std::move(E));
}
...
inline Error createFileError(const Twine &F, usize Line, std::error_code EC) {
...
public:
  ...
  ExitOnError(String Banner = "", int DefaultErrorExitCode = 1)
      : Banner(std::move(Banner)),
        GetExitCode([=](const Error &) { return DefaultErrorExitCode; }) {}

  /// Set the banner string for any errors caught by operator().
  void setBanner(String Banner) { this->Banner = std::move(Banner); }
