//===--- ItaniumDemangle.hpp ---------*- mode:c++;eval:(read-only-mode) -*-===//
...
#ifndef EXI_DEMANGLE_ITANIUMDEMANGLE_HPP
#define EXI_DEMANGLE_ITANIUMDEMANGLE_HPP

#include "DemangleConfig.hpp"
#include "StringViewExtras.hpp"
#include "Utility.hpp"
...
template <class T, size_t N> class PODSmallVector {
  ...
  void reserve(size_t NewCap) {
    size_t S = size();
    if (isInline()) {
      auto *Tmp = static_cast<T *>(exi::exi_malloc(NewCap * sizeof(T)));
      if (Tmp == nullptr)
        std::abort();
      std::copy(First, Last, Tmp);
      First = Tmp;
    } else {
      First = static_cast<T *>(exi::exi_realloc(First, NewCap * sizeof(T)));
      if (First == nullptr)
        std::abort();
    }
    Last = First + S;
...
#endif // EXI_DEMANGLE_ITANIUMDEMANGLE_HPP
