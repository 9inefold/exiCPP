//===--- Utility.hpp -----------------*- mode:c++;eval:(read-only-mode) -*-===//
...
#ifndef EXI_DEMANGLE_UTILITY_HPP
#define EXI_DEMANGLE_UTILITY_HPP
  ...
  void grow(size_t N) {
    ...
    if (Need > BufferCapacity) {
      ...
      if (BufferCapacity < Need)
        BufferCapacity = Need;
      Buffer = static_cast<char *>(exi::exi_realloc(Buffer, BufferCapacity));
      if (Buffer == nullptr)
        std::abort();
    }
  }
